...
-- =====================================================================
-- =====================================================================
-- 07-20-26 using Claude
-- =====================================================================
-- .Hammerspoon ARCHITECTURE VERSION CONTROL: 6.30.0-UNIVERSAL-COMMENTS
-- =====================================================================
-- Lee additions
...
--
--
-- NEW IN 6.30.0 — APP LOCK: panic key moved to hyper modal:
--   🔑 The panic key was ⌘⇧⌃⌥K (global hotkey). Now it is ⇪⇧H (hyper+shift+h),
--      integrated into the hyper modal like all other shortcuts. It still fires
--      instantly when held, same four-layer safety guarantee (panic key, watchdog,
--      correct PIN, process termination). Global hotkeys bypass the modal, so
--      ⇪⇧H works even if the modal is wedged (unclaimed hyper keys forward
--      exactly this chord).
--
...
--        1. Enter the PIN.
--        2. ⇪⇧H — panic key. Integrated into the hyper modal like all
--           shortcuts, but unclaimed hyper keys forward exactly this chord,
--           so it fires even if the modal is wedged. The panel says so,
--           on screen, in the text.
...
-- =====================================================================
-- WHAT EACH TOOL DOES :: ARCHITECTURE VERSION CONTROL: 6.29.1
-- =====================================================================
--
...
local function cheatSheetGroups()
    local groups = {
        ...
        { title = "🔒 APP LOCK (privacy screen, NOT security)", entries = {
            { "⇪⇧L", "Manage — Enter locks or unlocks the selected app" },
            ...
            { "🔁 row in ⇪⇧L", "Re-lock when you switch away (off by default)" },
            ...
            { "⇪⇧H", "Panic key — clears a stuck lock cover" },
        }},
...
-- =====================================================================
-- 6.6 APP LOCK — ⇪⇧L, a PIN gate on chosen apps
-- =====================================================================
...
-- just because you installed a new init.lua. Add apps from the ⇪⇧L
...
-- YOU lock it again from ⇪⇧L. 6.21 expired the unlock after 15 minutes,
...
-- you never see the window disappear. The point is that focus lands
-- somewhere PREDICTABLE (Finder) instead of wherever macOS decides,
-- which is what dragged your view to the other monitor and back.
-- Set to false to leave the app sitting under the cover instead.
local appLockCoverThenHide = true
...
-- false: a PIN unlocks the app until you re-lock it from ⇪⇧L. That is
...
-- Persisted in applock.json and toggleable from ⇪⇧L, so you can try both
...
--   2. ⇪⇧H — panic key, clears every cover instantly. It is in the hyper
--      modal, but unclaimed hyper keys forward the chord, so it works even
--      if the modal is confused.
            ...
            if ok and c then
                pcall(function()
                    c:appendElements(
                        ...
                        { type = "text",
                          text = "Enter your PIN   ·   ⇪⇧H clears this if anything goes wrong",
                          textSize = 15, textColor = { white = 1, alpha = 0.55 },
                          textAlignment = "center",
                          frame = { x = 0, y = f.h * 0.42 + 56, w = f.w, h = 30 } })
                end)
                -- Above ordinary windows, but NOT so high that it buries
                -- the PIN chooser: a cover you cannot type past is the
                -- lock-out this whole section is designed to avoid.
                pcall(function() c:level(hs.canvas.windowLevels.overlay) end)
...
local function appLockChallenge(name, app, screenHint)
    ...
    appLockVerifyHidden(app, name, function() return unlocked[name] end, function()
    appLockLog(name, "hidden — showing PIN prompt")
    ...
    _G.appLockWatcher = hs.application.watcher.new(function(name, event, app)
        ...
        if event == hs.application.watcher.deactivated then
            ...
            if appLockRelockOnLeave() and appLockIsLocked(name)
               and appLockIsUnlocked(name) then
                unlocked[name] = nil
        ...
        if not appLockHasPin() then
            ...
            print("⚠️ App Lock: " .. name .. " is marked locked but no PIN is set — allowing it. Set one with ⇪⇧L.")
            return
        end
        if app then
...
_G.choosers.appLock = hs.chooser.new(function(c)
    ...
    elseif c.action == "unprotect" then
        -- PIN-gated on purpose. Without this, anyone could open ⇪⇧L,
...
_G.hyperAddShortcut({"shift"}, "l", function()
    appLockManagerMode = "main"   -- always open on the main list
    appLockReopenManager()
end, "app lock manager")

-- ---- guarantees that a cover can never strand you -------------------
-- PANIC KEY. A hyper modal binding (⇪⇧H), but unclaimed hyper keys
-- forward the raw chord, so it works even if the modal is wedged.
_G.hyperAddShortcut({"shift"}, "h", function()
    ...
    if had then print("🔒 App Lock: covers cleared by the panic key (⇪⇧H)") end
end, "app lock panic")
...
_G.appLockStatusForBoot = function()
    ...
    if not appLockHasPin() then
        return (n > 0) and (n .. " apps marked but NO PIN SET — not locking (⇪⇧L to set one)")
               or "no PIN set, nothing locked (⇪⇧L to set up)"
    end
    local mode = appLockRelockOnLeave() and "re-locks when you switch away"
                 or "stays unlocked until you re-lock by hand"
    mode = mode .. (appLockUseCover and " · cover mode (⇪⇧H clears)" or " · hide mode")
    return (n == 0) and "PIN set, no apps locked yet (⇪⇧L to add)"
...
print("📌 init.lua ARCHITECTURE VERSION: 6.29.1")
