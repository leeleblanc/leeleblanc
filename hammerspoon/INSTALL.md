...
├── core/                 8 files, loaded directly by init.lua
│   ├── diagnostics.lua       ⇪⇧D
│   ├── cheatsheet.lua        ⇪/
...
├── modules/              34 files, loaded by the §1.12 loader
...
Syntax on `init.lua`, `core/` and all 34 modules, twenty-four Lua suites, then the
