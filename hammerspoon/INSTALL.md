...
├── core/                 7 files, loaded directly by init.lua
│   ├── diagnostics.lua       ⇪⇧D
│   ├── cheatsheet.lua        ⇪/
...
├── modules/              32 files, loaded by the §1.12 loader
...
Syntax on `init.lua`, `core/` and all 32 modules, twenty-one Lua suites, then the
