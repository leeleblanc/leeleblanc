...
function M.setup(core)
    ...
    local function logIt(path, what)
        dk.log[#dk.log + 1] = { when = os.date("%H:%M:%S"),
                                ...
                                what = what }
        if #dk.log > 40 then table.remove(dk.log, 1) end
    end
        ...
        local okNew, t = pcall(function() return hs.task.new("/usr/bin/unzip",
            function(exitCode, stdOut)
                ...
                local okAll = pcall(function()
                    ...
                    if comment then dk.writeComment(path, comment) end
                end)
                if not okAll then
    ...
    function dk.flush()
        dk.timer = nil
        local batch = dk.pending
        dk.pending = {}
        for path in pairs(batch) do pcall(dk.process, path) end
    end
    ...
    function _G.docKeywordsReport()
        local L = { "🏷 DOC KEYWORDS this session — newest last" }
