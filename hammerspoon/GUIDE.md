...
| Volume changed my whole Mac, not the app | That is the 🌐 label in the alert, and it is macOS, not a bug — only Music and Spotify (🎯) can move their own volume. Real per-app mixing needs a driver: BackgroundMusic (free) or SoundSource, both needing admin |
