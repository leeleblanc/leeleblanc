...
if [ ! -d "$HS/core" ]; then
  ...
  printf "   %-22s %7s  %-16s %s\n" NAME BYTES MODIFIED NOTE
  for n in diagnostics cheatsheet boot_report boot_cost key_trail capabilities notices coexist hyper_key changelog_csv console lag double_tap; do
    f="$HS/core/$n.lua"
    if [ -f "$f" ]; then
      printf "   %-22s %7s  %-16s %s\n" "$n" \
  ...
  echo "   count: $(ls -1 "$HS/core"/*.lua 2>/dev/null | wc -l | tr -d ' ') files (expect 13)"
fi
